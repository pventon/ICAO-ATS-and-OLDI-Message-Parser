...
# (spaces, newlines, carriage return and tabs). The Tokenizer produces a list
# of tokens in the Tokens class; this class uses the tokens as input to parse.
# The output is stored in an instance of the ExtractedRouteRecord class that is
# populated by this parser class.
# The parser checks for field 15 grammar and semantics reporting errors if
# necessary.
...
# ICAO field 15.
class ParseF15:
    # The default speed used to assign a speed when a speed is not
    # given, e.g. such as when a SPEED/VFR element is processed.
    # Default is set to F050, (Flight Level 50).
    ...
    # The records in an instance of ExtractedRouteRecord class contains a complete extracted
    # route with all points, routes, flight rules, speed and altitudes etc. along a route.
    # Arguments
    # ---------
    # ers:      An instance of ExtractedRouteSequence class being populated by the parser;
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    ...
    # Arguments
    # ---------
    # ers:          An instance of ExtractedRouteSequence class into which the
    #               erroneous token is being stored;
    # tokens:       A list of tokens extracted from field 15 used as input to the
    #               parser. This structure contains a tokenized form of all field
    #               15 tokens used as input to this parser.
    # token:        The erroneous token;
    # error_number: An integer value representing an index to an error message
    ...
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance that this token is being
    #           appended to;
    # token:    The token being saved;
    ...
    # This method saves the altitude data to the imperial and SI altitude
    # attributes of an ERS record. All four altitude types encountered
    # in a field 15 are converted to meters for SI storage. The altitude
    # types processed are:
    # F - Flight level
    ...
    # Arguments
    # ---------
    # ers:              An ExtractedRouteSequence class instance that an error may
    #                   be written to if an error is detected;
    # token:            A token containing a token used to report an error if an error
    #                   is detected in the token.
    # ex_route_rec:     An ExtractedRouteRecord record that the altitude is written to;
    # altitude_string:  An altitude as a string extracted from field 15, e.g. F350;
    # cruise:           A boolean indicating if the altitude_string is part of a
    #                   cruise / climb token.
    # return:           Void
    ...
    # This method sets the bearing / distance on a point when two consecutive points
    # are found both with valid latitude and longitude values assigned.
    # This method 'looks' backward in the ERS to locate a 'previous' point
    # with a valid latitude and longitude assigned. Points can follow one another
    # or be separated by a connector (e.g. such as a DCT, ATS Route etc.).
    # Arguments
    # ---------
    # ers:      The ERS whose last point has just been assigned a latitude / longitude.
    # return:   Void
    ...
    # This method assigns a floating point number to a points' latitude. The
    # method takes the latitude input as a string.
    # Arguments
    # ---------
    # ex_route_rec: An extracted route record containing a point whose latitude
    #               is being set.
    # latitude:     The latitude as a string; 2 characters for degrees, 4 characters
    ...
    # This method processes a point looking for and checking the semantics
    # for latitude / longitude and bearing distance points.
    # Errors are reported as they are detected; the parser does not reset when
    # an error is located but continues to detect any and all semantic errors
    # in a given point. Semantic checks ensure angles do not exceed those specified
    # as follows:
    ...
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance that any erroneous token are saved to;
    # token:    The point token being semantically checked;
    # return:   Void
    ...
    # This method takes a field 15 element representing a point defined as a
    # latitude / longitude in degrees (no minutes) and converts the string
    # representation of the latitude / longitude into decimal values and
    # stores them in an ERS record.
    # The North / South and East / West indicators determine if the decimal
    # value is negative or positive.
    # Errors are reported if the angular values specify incorrect semantics.
    # Valid angles are:
    ...
    # Arguments
    # ---------
    # ers:          The complete Extracted Route Sequence (ERS);
    # token:        The field 15 lat/long element being processed;
    # ex_route_rec: An ERS record into which the decimal lat/long values will
    #               be written
    # token_string: The string representing an ICAO lat/long (e.g.23N123E)
    # return:       Void
    ...
    # This method takes a field 15 element representing a point defined as a
    # latitude / longitude in degrees (with minutes) and converts the string
    # representation of the latitude / longitude into decimal values and
    # stores them in an ERS record.
    # The North / South and East / West indicators determine if the decimal
    # value is negative or positive.
    ...
    # This method assigns a floating point number to a points' longitude. The
    # method takes the longitude input as a string.
    # Arguments
    # ---------
    # ex_route_rec: An extracted route record containing a point whose longitude
    #               is being set.
    # latitude:     The longitude as a string; 3 characters for degrees, 5 characters
    #               for degrees and minutes.
    # return:       Void
    @staticmethod
    ...
    # This method saves the speed data to the imperial and SI speed
    # attributes of an ERS record. All three speed types encountered
    # in a field 15 are converted to meters / second for SI storage.
    # The speed types processed are:
    # K - Kilometres per hour
    # N - Knots
    # M - True Mach number
    # The speeds are applied to the point at which a speed is encountered
    # and propagated forward until a subsequent speed change is encountered.
    # Note that the Mach speed requires the altitude, hence the ERS altitude
    # record must always be set before determining the speed. Some field 15
    # elements contain only a speed (e.g. Speed / VFR). If a Mach number is
    # given in such elements, the speed is calculated using the speed of sound
    # at sea level.
    # Arguments
    # ---------
    # ex_route_rec: An ExtractedRouteRecord record that the altitude is written to;
    # speed_string: A speed as a string extracted from field 15, e.g. N0450;
    # return:   Void
    @staticmethod
    ...
    # This method processes a speed / altitude element, (e.g. N0450F350).
    # A speed / altitude element is always preceded by a point, hence the speed and
    # altitude are applied to the preceding point which is the last ERS record.
    # Note that the altitude must be assigned in meters to the ERS before the
    # speed as the speed given as a Mach number uses the altitude to calculate
    # a speed.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing a point in
    #           the last ERS record that the speed and altitude will be written to.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    The speed / altitude token from which the speed and altitude will
    #           be extracted from;
    # return:   Void
    ...
    # This method processes the speed / altitude / altitude part of a cruise climb
    # element.
    # A speed / altitude / altitude element is always preceded by a point, hence
    # the speed and altitude are applied to the preceding point which is the
    # last ERS record.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing a point in
    ...
    # This method processes the speed / altitude / plus part of a cruise climb
    # element.
    # A speed / altitude / plus element is always preceded by a point, hence
    # the speed and altitude are applied to the preceding point which is the
    # last ERS record.
    # Arguments
    # ---------
    ...
    # This method precess a speed / VFR element.
    # The element preceding a SPEED/VFR token must be a point, hence we
    # have to set the speed at the previous point and assign VFR rules 
    # at a new ERS record to store the rule change VFR record.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing a point in
    #           the last ERS record that the speed will be written to and a new
    #           VFR record will be appended to.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    The speed / VFR token from which the speed will be extracted from;
    # return:   Void
    ...
    # This method is processing a 'break' end token, one of 'IFR', 'GAT' or
    # 'IFPSTART'. A 'break' is considered to be a break in IFR routing,
    # i.e. a change from IFR to VFR and back to IFR has a 'break' between two
    # IFR sections. Any tokens appearing between the end of the first IFR section
    # and the start of the second IFR section are stored on the intermediate VFR
    # (or OAT, IFPSTOP) section token, (referred to as break text).
    # If the end of a break section is NOT occurring then control is returned
    # to saving these tokens as 'break' text.
    # All break text ends must be followed by a point. The IFR end must be further
    # followed by a point/speed/altitude or point/speed/vfr element.
    # As a rule change is terminated by a point, control is handed to the point
    # node for further processing.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing a start break
    #           token, (one of VFR, OAT or IFPSTOP) in the last ERS record that
    #           the break text will be written to, or if the 'break' is determined
    #           then the change back to IFR will be appended to the ERS.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    The 'break' end token, ('IFR', 'GAT' or 'IFPSTART' token indicating
    #           the end of 'break';
    # return:   Void
    ...
    # This method reports an error if the end of a 'break' section does not match
    # a 'break' start token. A 'break' is considered to be a break in IFR routing,
    # i.e. a change from IFR to VFR and back to IFR has a 'break' between two IFR
    # sections. Break sections are indicated by start/end matching pairs VFR/IFR,
    # OAT/GAT, IFPSTOP/IFPSTART. Erroneous case are, e.g. a VFR break started and
    # now a GAT 'break' end is detected, hence the start and end indicators do not
    # form part of a matching 'break' indicator pair.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing a point
    #           token in the last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    The 'break' end token, ('IFR', 'GAT' or 'IFPSTART' token indicating
    ...
    # This method is processing the elements 'OAT', IFPSTOP', or 'VFR' all of
    # which indicate the start of non-IFR routing, a 'break' section.
    # A 'break' is considered to be a break in IFR routing, i.e. a change from IFR to
    # VFR and back to IFR has a 'break' between two IFR sections.
    # The ERS last element is a point at which the 'break' section starts. The start
    # 'break' token will be appended to the ERS and one of the following flight
    # rules set on the start 'break' token:
    # OAT -> rules set to 'O'
    # VFR -> rules set to 'V'
    ...
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    The 'break' start token, ('VFR', 'OAT' or 'IFPSTOP' token indicating
    #           the start of 'break' section;
    # return:   Void
    ...
    # This method loops over elements saving them as 'break' text; break text follows
    # the 'break' start tokens VFR, OAT or IFPSTOP. A 'break' is considered to be a
    # break in IFR routing, i.e. a change from IFR to VFR and back to IFR has a
    # 'break' between two IFR sections. Once a 'break' end element is detected,
    # processing takes place to ascertain if a 'break' section is being correctly
    # terminated; if not, this method continues to save the tokens as 'text' on
    # the 'break' start token. If there is a possible 'break' end detected control
    # is passed to self.break_end().
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing a 'break' start
    #           token in the last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    The 'break' text token being saved to the 'break' start token;
    # return:   Void
    ...
    # This method copies the speed, altitude and flight rules from the 'previous'
    # ERS token to the 'current' (last) ERS token.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing the last ERS
    #           record that has to have its speed, altitude and flight rule
    #           attributes copied from the penultimate ERS record to the last
    #           record.
    # return:   Void
    @staticmethod
    ...
    # This method processes the 'C' token that indicates a cruise climb element
    # may be present. If the token following the 'C' is a '/' then we assume a
    # cruise climb token has been located, in such a case, the 'C' is not stored
    # in the ERS. If the token following 'C' is not a '/' then the 'C' is considered
    # to be a single character point and stored in the ERS as a point.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing a 'break' start
    #           token in the last ERS record.
    ...
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an IFR routing
    #           element in the last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    The point in a cruise/climb element;
    # return:   Void
    ...
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an IFR routing
    #           point element in the last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    The DCT element;
    # return:   Void
    ...
    # This method processes a '/' token; the '/' character is not stored in the ERS.
    # The method looks for tokens following the '/' as there are only certain element
    # types allowed to follow a '/', namely speed/vfr, speed/altitude, truncate indicator
    # point or route. All other element types are incorrect resulting in an error
    # being reported.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an IFR point
    #           element in the last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    ...
    # This method determines the next node to move to after a speed/altitude
    # has been applied to the first ERS record, the ADEP element.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing only the ADEP
    #           record element in the last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    The first token in the tokens list.
    # return:   Void
    ...
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an IFR element
    #           record in the last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    A token being checked if it can follow an IFR point;
    # return:   Void
    ...
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an SID element
    #           record in the last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    A token being checked if it can follow an SID element;
    # return:   Void
    ...
    # This method processes a point element. Points are always IFR elements, hence
    # the rules are always set to IFR on point elements and stored in the ERS.
    # The angle semantics for Latitude / Longitude and bearing distance points are
    # checked with appropriate errors reported if semantic errors exist.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an IFR element
    #           record in the last ERS record.
    ...
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    A point token being appended to the ERS;
    # return:   Void
    ...
    # This method calculates the coordinates for a point given by a
    # Lat / Long / Bearing / Distance
    # Arguments
    # ---------
    # ers:          The complete extracted route sequence
    # ex_route_rec: The extracted route record containing the lat/long of the
    #               point to which the bearing / distance relate.
    # bearing:      The bearing from the point along which the point to be calculated
    #               lies.
    # distance:     The distance along the bearing where the point lies.
    # return:       Void
    ...
    # This method attempts to re-synchronize the parser after an error is reported.
    # This method is called whenever an error is reported / added by the
    # 'self.add_error()' method. The next token is retrieved and based on its type,
    # after which parsing continues based on the next tokens element type.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an erroneous element
    #           record in the last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    ...
    # This method processes an ATS route element. Routes are always IFR elements,
    # hence the rules are always set to IFR on route elements and stored in the ERS.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an IFR element
    #           record in the last ERS record.
    ...
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    An ATS route token being appended to the ERS;
    # return:   Void
    ...
    # This method sets a bearing / azimuth and distance from a point 'point_1' to
    # point 'point_2'.
    # Arguments
    # ---------
    # point_1:  The point that will have the bearing / azimuth and distance set
    #           that provides the azimuth and distance from this point to point_2.
    # point_2:  The second point to calculate the azimuth and distance to.
    # return:   Void
    @staticmethod
    ...
    # This method processes an SID token that must be the token following
    # the ADEP. Any other location in field 15 will result in an error.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing only the ADEP
    #           (no error case) or any other element type (error case) record in
    #           the last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    An SID token being appended to the ERS;
    # return:   Void
    ...
    # This method processes an element which matches the syntax for both a SID or
    # STAR element; the syntax cannot be used to uniquely identify which element
    # type it is. The exact type can only be determined by its position in field 15.
    # The SID must be the first token in the list of tokens and the STAR the last.
    # If the element appears anywhere else in the token list it is an error.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing only the ADEP
    #           or an IFR element that is assumed to be the penultimate element in
    #           field 15 as the last ERS record (no error case) or the SID/STAR
    #           element is in the middle of the field 15 token list (error case).
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    An SID or STAR token being appended to the ERS;
    # return:   Void
    ...
    # This method processes a STAR token that must be the last token in the list
    # of tokens. Any other location in field 15 will result in an error.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an IFR element
    #           as its last record that is assumed to be the penultimate element in
    #           field 15 (no error case) or the STAR element is in the middle of
    #           the field 15 token list (error case).
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    A STAR token being appended to the ERS;
    # return:   Void
    ...
    # This method processes a STAY token that indicates a 'stay' time at an IFR
    # point preceding the STAY token. The method checks the correct token sequence
    # i.e. Stay -> '/' -> HHMM. Errors are reported if the sequence is incorrect.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an IFR point element
    #           as its last ERS record.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    A STAY token being appended to the ERS;
    # return:   Void
    ...
    # This method processes a stay HHMM token that provides the duration at a 'stay'
    # point. The method saves the HHMM token to the ERS and determines the next
    # processing node after the HHMM TOKEN.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing an IFR point element
    #           as its last ERS record on which the stay time will be stored in minutes.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    A stay HHMM token being saved on the last ERS point record;
    # return:   Void
    ...
    # This method processes the 'T' truncate field 15 token. The 'T' character
    # indicates that the field 15 has been truncated. No elements should occur
    # after this element. The 'T' is not saved to the ERS. If there are any other
    # tokens following the 'T' an error is reported.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance containing the last processed
    #           element irrespective of its type.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # token:    A T token being processed to determine if field 15 is being correctly truncated;
    # return:   Void
    ...
    # This method executes a rule change from VFR to IFR. To complete this rule
    # change there has to be a point, a slash '/' and a SPEED/LEVEL following
    # the IFR rule change element. The method break_end() performs a look-ahead from
    # the IFR token to ensure these tokens are present and then calls this method.
    # This method assumes all the required tokens are available in the Tokens list
    # to copy the rule change elements to the ERS.
    # Arguments
    # ---------
    # ers:      An ExtractedRouteSequence class instance with the last record
    #           containing the VFR item onto which all and any break text are copied.
    # tokens:   A list of tokens extracted from field 15 used as input to the parser.
    #           This structure contains a tokenized form of all field 15 tokens used as
    #           input to this parser.
    # return:   Void
    ...
    # This method processes a rule change from VFR to IFR occurs but the IFR
    # point changes back to VFR, i.e. a single point IFR section.
    # This method executes a rule change from VFR to IFR and back to VFR. To complete
    # this rule change there has to be a point, a slash '/' and a SPEED/VFR following
    # the IFR rule change element. The method break_end() performs a look-ahead from
    # the IFR token to ensure these tokens are present and then calls this method.
    # This method assumes all the required tokens are available in the Tokens list
    # to copy the rule change elements to the ERS.
